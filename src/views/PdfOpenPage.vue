...
const getPdfFromRoute = async () => {
  isLoading.value = true;
  try {
    if (folder && filePath) {
      fileUrl.value = `${URL}/${folder}/${filePath}`;
      await renderPdf(fileUrl.value);
    } else {
      console.error('Folder or filePath not found in route params');
      fileUrl.value = '';
    }
  } catch (error) {
    console.error('Error loading PDF from route params:', error.message);
    fileUrl.value = '';
  } finally {
