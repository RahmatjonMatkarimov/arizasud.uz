...
const activeSection = ref('status4'); // Default section: Qabul qilingan fayllar (status4)
...
const statusCards = [
  {
    id: 1,
    title: 'Kerakli hujjatlar yig\'ib chiqish',
    count: 0,
    documentId: '',
    status: 'accepted',
    color: 'success',
  ...
  {
    id: 2,
    title: 'Imlo xatolarini tekshirish',
    count: 0,
    documentId: 'N/A',
    status: 'rejected',
    color: 'error',
  ...
  {
    id: 3, // Fixed duplicate id (changed from 2 to 3)
    title: 'To\'liq to\'lovlarni kutush jarayonida',
    count: 0,
    documentId: 'N/A',
    status: 'rejected',
    color: 'as',
...
const dynamicStatusCards = computed(() => {
  ...
  tasks.forEach((task) => {
    ...
    if (statusNumber === '4') counts.status4 += 1;
    else if (statusNumber === '6') counts.status6 += 1;
    else if (statusNumber === '7') counts.status7 += 1;
  });
  ...
  try {
    ...
    const filteredTasks = fetchedData.LawyerTask.filter((item) => {
      if (item.ClientFileStatusHistory && item.ClientFileStatusHistory.length > 0) {
        ...
        return ['4', '6', '7'].includes(latestStatus); // Only status4 and status6
      }
      return false;
    });
...
const moveToNextStage = (id) => {
  ...
  if (activeSection.value === 'status7') {
  ...
  } else if (activeSection.value === 'status4') {
    selectedDocId.value = id;
    selectedDoc.value = filteredDocuments.value.find((doc) => doc.id === id) || null;
    isAdminModalOpen.value = true;
    getAdminFromSubDomain(); // Faqat status7 uchun subdomen ma'lumoti
  } else {
...
const filteredDocuments = computed(() => {
  ...
  return tasks
    ...
    .map((task) => ({
      ...
      status: task.ClientFileStatusHistory?.[task.ClientFileStatusHistory.length - 1]?.status || 'status4',
              ...
              <li v-for="(doc, index) in filteredDocuments" :key="doc.id">
                <div class="px-5 py-4 border-b border-gray-300 flex items-center space-x-3 cursor-pointer"
                  ...
                  <div class="flex-1">
                    ...
                    <div v-if="expandedDocId === doc.id" class="mt-4 w-[86%] 2xl:w-[100%]">
                                ...
                                <div :class="[
                                  ...
                                  <div class="mt-2 text-center">
                                    <div class="group relative">
                                      ...
                                      <p
                                        class="absolute -ml-[70%] hidden -mt-2 rounded-md z-40 bg-gray-600 group-hover:block min-w-[300px] text-center">
                                        {{ dat === 'datakril' ? translateText(step.title) : step.title }}
                                      </p>
                                    </div>
                      ...
                      <div class="mt-4 -mr-[170px] flex justify-end space-x-2">
