  ...
  <div class="min-h-screen bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200/70">
    ...
    <div class="flex justify-center items-center pt-4 px-4">
      <h1
        ...
        {{ dat === 'datakril' ? translateText("Operatorlar ro'yxati") : "Operatorlar ro'yxati" }} <span v-if="Count"
...
const getData = async () => {
  ...
  try {
    ...
    const response = await axios.get(`${URL}/operator`, {
...
isLoading.value = true
  try {
    const token = localStorage.getItem("token");
    await axios.post(`${URL}/operator`, formData, {
      headers: {
...
isLoading.value = true
  try {
    const token = localStorage.getItem("token");
    await axios.put(`${URL}/operator/${selectedId.value}`, formData, {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "multipart/form-data",
      },
...
isLoading.value = true
  try {
    const token = localStorage.getItem("token");
    await axios.put(
      `${URL}/operator/${selectedId.value}/password`,
      {
...
isLoading.value = true
  try {
    const token = localStorage.getItem("token");
    await axios.delete(`${URL}/operator/${selectedId.value}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
