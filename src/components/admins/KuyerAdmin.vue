  ...
  <div class="min-h-screen flex flex-col items-center p-4">
    ...
    <div class="flex justify-center mb-16 mt-4 items-center">
      <h1 class="text-black text-[40px] font-bold text-center bg-lime-500 border-[3px] border-black rounded-lg py-2 px-[100px]">
         {{ dat === 'datakril' ? translateText('Kuryerlar ro\'yxati') : 'Kuryerlar ro\'yxati' }}<span v-if="Count" class="border-b-4 px-1 border-black text-black">{{ Count }}</span>
      </h1>
    </div>
...
const updateAdmin = async () => {
  ...
  try {
    const token = localStorage.getItem("token");
    await axios.put(`${URL}/deliverer/${selectedId.value}`, formData, {
      headers: {
...
const updatepassword = async () => {
  ...
  try {
    const token = localStorage.getItem("token");
    await axios.put(
      `${URL}/deliverer/${selectedId.value}`,
      {
...
const post = async () => {
  ...
  try {
    const token = localStorage.getItem("token");
    await axios.post(`${URL}/deliverer`, formData, {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "multipart/form-data",
      },
...
const removeAdmin = async () => {
  ...
  try {
    const token = localStorage.getItem("token");
    await axios.delete(`${URL}/deliverer/${selectedId.value}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
