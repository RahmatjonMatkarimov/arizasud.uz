      ...
      <div class="bg-white p-6 rounded-lg w-11/12 max-w-md">
        <div class="flex justify-end">
          <img @click="qwe = false" class="w-8 -m-4 cursor-pointer absolute" src="../../../public/reject.png"
            :alt="$t('createModal.closeAlt')" />
        </div>
        <input v-model="name" class="w-full text-black p-2 mb-3 border rounded-lg"
          :placeholder="$t('createModal.namePlaceholder')" />
        <input v-model="surname" class="w-full text-black p-2 mb-3 border rounded-lg"
        ...
        <input v-model="jshhr" class="w-full text-black p-2 mb-3 border rounded-lg"
        ...
        <input v-model="role" class="w-full p-2 mb-3 border text-black rounded-lg"
          :placeholder="$t('createModal.rolePlaceholder')" />
        <input v-model="username" class="w-full text-black p-2 mb-3 border rounded-lg"
          :placeholder="$t('createModal.usernamePlaceholder')" />
        <div class="relative">
          <input v-model="password1" :type="showPassword ? 'text' : 'password'"
            class="w-full text-black p-2 mb-3 border rounded-lg pr-10"
    ...
    <div v-if="PutModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white p-6 rounded-lg w-11/12 max-w-md">
        ...
        <input v-model="updatedBirthday" type="date" class="w-full text-black p-2 mb-3 border rounded-lg"
          :placeholder="$t('createModal.birthdayPlaceholder')" />
        <input v-model="updatedrole" class="w-full p-2 mb-3 border text-black rounded-lg"
          :placeholder="$t('createModal.rolePlaceholder')" />
...
const post = async () => {
  ...
  try {
    const token = localStorage.getItem("token");
    const response = await axios.post(`${URL}/yurist`, formData, {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "multipart/form-data",
      },
...
const removeAdmin = async () => {
  ...
  try {
    const token = localStorage.getItem("token");
    const response = await axios.delete(`${URL}/yurist/${selectedId.value}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
