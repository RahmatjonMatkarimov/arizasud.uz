        ...
        <ul class="list-none p-0 m-0">
          <li class="relative">
            <div @click.stop class="">
              <h4 class="text-[30px] mb-6 font-semibold text-center text-gray-800">
                {{
                  dat === "datakril"
                    ? translateText(
                        "Ish jarayoni holat \"Sudga taqdim etildi\" bo'lganida to'liq tugallangan hisoblanadi!"
                      )
                    : "Ish jarayoni holat \"Sudga taqdim etildi\" bo'lganida to'liq tugallangan hisoblanadi!"
                }}
              </h4>
                ...
                <div class="inline-block min-w-full">
                  ...
                  <template v-for="(file, fileIndex) in filteredFiles" :key="file.id">
                    <div class="lg:flex overflow-x-auto items-center">
                      ...
                      <template
                        v-for="(step, index) in getUniqueStatuses(file.LawyerTask)"
                        :key="step.id"
                      >
                        <div
                          :class="[
                        ...
                        <div class="flex justify-center">
                          ...
                          >
                            <div
                              class="w-[70px] rotate-90 lg:rotate-180 h-5 overflow-hidden"
                            >
                              <div
                                class="h-full bg-lime-600 bg-stripes"
                                style="width: 100%"
                              ></div>
                            </div>
                          </div>
      ...
      >
        ...
        <div class="mb-4">
          ...
          <span class="text-red-600 font-medium" v-else>
            ...
            {{ selectedPaymentDetails[selectedPaymentDetails.length - 1].remainingSum
...
const getStatus = (status) => {
  ...
  const statusMap = {
    ...
    status7:
      "Davlat boji va boshqa to'lovlarni buyurtmachi to'liq to'lashini kutish jarayonida",
...
const getUniqueStatuses = (lawyerTask) => {
  ...
  if (statuses.length > 0) {
    ...
    statuses.forEach((status, index) => {
      status.isLatestRejectionWithComment =
        index === statuses.length - 1 && !isLastCommentEmpty;
    });
  }
