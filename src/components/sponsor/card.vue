<template>
  <div class="w-full flex flex-col px-[15px] justify-center items-center mt-10">
    <div class="rounded-2xl w-full relative p-10 mb-16 overflow-hidden bg-gray-200">
      ...
      <button @click="func()" class="bg-lime-500 text-[20px] rounded-lg py-2 absolute top-4 right-4 px-4">barchasini ko'rish</button>
      ...
      <Carousel v-if="asd" :items-to-show="4" :autoplay="1500" wrap-around class=" p-[20] w-full">
        <Slide v-for="item in currentData" :key="item.id">
          <div
            class="bg-white border-[#223B9E] flex flex-col justify-start items-center gap-2 border-[5px] break-words w-72 min-h-full rounded-xl relative hover:-translate-y-3 duration-500 hover:shadow-[0px_0px_50px_5px_rgba(255,255,255,1)] p-2 mx-auto">
            <div class="flex justify-center items-center p-2 h-[160px]">
