...
const permissions = [
  ...
  { key: 'ticket', label: "Ishchilarni bajargan ishlarini ro'yxatini ko'rish uchun ruxsat" },
    ...
    <div class="max-w-[1140px] mx-auto p-6">
      <div class="bg-gradient-to-r relative from-purple-900 to-purple-600 w-full h-[200px] rounded-t-lg">
        <div :class="adminStatus.color" class="flex items-center absolute top-4 right-4">
            <span :class="adminStatus.dotColor" class="w-[25px] border-2 border-white h-[25px] rounded-full shadow-md"></span>
         </div>
      </div>
      <div class="relative bg-white dark:bg-gray-700 rounded-b-lg shadow-lg">
        ...
        <div class="pt-20 px-6 pb-6">
          <div :class="adminStatus.color" class="flex items-center">
            <span>{{ dat === 'datakril' ? translateText(adminStatus.status) : adminStatus.status }}</span>
          </div>
          <div class="flex justify-between items-center">
            <div>
              <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">
                {{ dat === 'datakril' ? translateText(`${userInfo.name} ${userInfo.surname}`) : `${userInfo.name} ${userInfo.surname}` }}
              </h1>
            </div>
          </div>
<div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
  ...
  <div class="flex items-center text-gray-700 dark:text-gray-200">
    ...
    <span>{{ dat === 'datakril' ? translateText(userInfo?.phone) : userInfo?.phone }}</span>
  </div>

  <!-- Tug‘ilgan sana -->
  <div class="flex items-center text-gray-700 dark:text-gray-200">
    <Icon icon="mdi:cake-variant" class="w-5 h-5 mr-2 text-blue-500" />
    <span>{{ dat === 'datakril' ? translateText(Timeee(userInfo?.birthday)) : Timeee(userInfo?.birthday) }}</span>
  </div>

  <!-- Lavozimi -->
  <div class="flex items-center text-gray-700 dark:text-gray-200">
    <Icon icon="mdi:briefcase-account" class="w-5 h-5 mr-2 text-blue-500" />
    <span>{{ dat === 'datakril' ? translateText(userInfo?.lavozimi) : userInfo?.lavozimi }}</span>
  </div>

  <!-- Ish boshlagan vaqti -->
  <div class="flex items-center text-gray-700 dark:text-gray-200">
    <Icon icon="mdi:calendar-clock" class="w-5 h-5 mr-2 text-blue-500" />
    <span>{{ dat === 'datakril' ? translateText(`Ish boshlagan vaqti: ${Timeee(userInfo?.startWork)}`) : `Ish boshlagan vaqti: ${Timeee(userInfo?.startWork)}` }}</span>
  </div>

  <!-- ID karta raqami -->
  <div class="flex items-center text-gray-700 dark:text-gray-200">
    <Icon icon="mdi:card-account-details-outline" class="w-5 h-5 mr-2 text-blue-500" />
    <span>{{ dat === 'datakril' ? translateText('ID karta raqami:') : 'ID karta raqami:' }} {{ userInfo.userCode }}</span>
  </div>

  <!-- JSHSHIR -->
  <div class="flex items-center text-gray-700 dark:text-gray-200">
    <Icon icon="mdi:barcode" class="w-5 h-5 mr-2 text-blue-500" />
    <span>{{ dat === 'datakril' ? translateText('JSHSHIR:') : 'JSHSHIR:' }} {{ userInfo.uniqueCode }}</span>
  </div>
</div>
      ...
      <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="bg-white dark:bg-gray-700 p-4 rounded-lg shadow-lg">
          <h2 class="text-lg font-semibold mb-4">{{ dat === 'datakril' ? translateText('Ruxsatlar') : 'Ruxsatlar' }}</h2>
          <ul class="space-y-4">
        ...
        <div class="bg-white dark:bg-gray-700 p-4 rounded-lg shadow-lg">
          <h2 class="text-lg font-semibold mb-4">{{  dat === 'datakril' ? translateText('Qo\'shimcha ma\'lumotlar') :'Qo\'shimcha ma\'lumotlar' }}</h2>
          <ul class="space-y-4">
