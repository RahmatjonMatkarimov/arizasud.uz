<template>
  <div class="dark:bg-[#1a2642] bg-gray-200 dark:text-gray-200 p-7 min-h-screen">
    <div
      class="dark:bg-black dark:bg-opacity-20 bg-white container mx-auto rounded-lg shadow-2xl p-6"
    >
      <div class="flex justify-between items-center mb-6 pb-4 border-b">
        <div
          class="text-black dark:text-blue-400 font-medium cursor-pointer"
      ...
      <div>
        <div v-for="item in users" :key="item.id">
            ...
            <div class="flex relative justify-between w-full">
              <div class="flex flex-col w-full gap-1">
                ...
                <h1 class="text-md">
                  <span class="text-md font-bold"
                    >{{ dat === "datakril" ? translateText("F.I.O:") : "F.I.O:" }}
                  </span>
                  {{
                    dat === "datakril"
                      ? translateText(`${item.name} ${item.surname} ${item.dadname}`)
                ...
                <h1 class="text-md">
                  <span class="text-md font-bold"
                    >{{
                      dat === "datakril"
                        ? translateText("Ishchini telefon raqami:")
                        : "Ishchini telefon raqami:"
                    }}
                  </span>
                  {{ item.phone }}
                </h1>
                <h1 class="text-md">
                  <span class="text-md font-bold"
                    >{{
                      dat === "datakril"
                        ? translateText("Ishchini lavozimi:")
                        : "Ishchini lavozimi:"
                    }}
                  </span>
              ...
              <div class="flex gap-6">
                <div
                  ...
                  :key="card.title"
                  @click="
                    card.modal ? openRejectModal() : null,
                      card.modal1 ? openModal('To\'liq hisob', $event) : null
                  "
                  :class="`2xl:w-[400px] w-[300px] h-[180px] rounded-xl shadow p-6 ${card.bg}`"
                >
                  <div class="flex items-center gap-2">
                    <img
                      class="w-[30px]"
                      :src="card.icon"
                      alt="{{ dat === 'datakril' ? translateText('Belgi') : 'Belgi' }}"
                    />
                  ...
                  <p :class="card.colorClass + ' text-3xl font-bold'">
                    {{ dots(card.value) }}
                    {{ dat === "datakril" ? translateText("so'm") : "so'm" }}
                  </p>
                  <p class="text-sm dark:text-white text-black mt-1">
                    <span class="font-semibold text-md">{{
                      dat === "datakril" ? translateText(card.increase) : card.increase
                    }}</span>
                  </p>
                </div>
              </div>
              ...
              <div class="flex absolute flex-wrap right-3 bottom-3 items-end">
                <div class="flex flex-wrap items-end gap-4">
                  ...
                  >
                    {{
                      dat === "datakril"
                        ? translateText("Bonus maoshni qo'shish")
                        : "Bonus maoshni qo'shish"
                    }}
                  </button>
                  <button
                    @click="handleCalculateSalary(item)"
                    class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700"
                  >
                    {{
                      dat === "datakril"
                        ? translateText("Oylik maoshni hisoblash")
                        : "Oylik maoshni hisoblash"
                    }}
                  </button>
                  <select
                    v-model="selectedYear"
                    class="border border-gray-500 dark:border-gray-200 bg-[#fff0] rounded-md p-2"
                  >
                    <option class="dark:text-white text-black" value="">
                      {{
                        dat === "datakril" ? translateText("Yil tanlang") : "Yil tanlang"
                      }}
                    </option>
                    <option
                  ...
                  >
                    <option class="dark:text-white text-black" value="">
                      {{
                        dat === "datakril" ? translateText("Oy tanlang") : "Oy tanlang"
                      }}
                    </option>
                    <option
                      class="dark:text-white text-black"
          ...
          <table class="w-full border-collapse mt-4">
            <thead>
              <tr
                class="w-full grid dark:bg-gradient-to-r from-[#2a3655] to-[#3d4e81] rounded-lg border-black/50 border shadow-lg hover:shadow-blue-500/5 duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-2xl bg-white object-cover grid-cols-6 gap-2 items-center"
              >
                <th class="p-3 text-center font-semibold">
                  {{
                    dat === "datakril" ? translateText("Ishga kelgan") : "Ishga kelgan"
                  }}
                </th>
                <th class="p-3 text-center font-semibold">
                  {{
                    dat === "datakril"
                      ? translateText("Tushlikga ketgan")
                      : "Tushlikga ketgan"
                  }}
                </th>
                <th class="p-3 text-center font-semibold">
                  {{
                    dat === "datakril"
                      ? translateText("Tushlikdan kelgan")
                      : "Tushlikdan kelgan"
                  }}
                </th>
                <th class="p-3 text-center font-semibold">
                  {{
                    dat === "datakril"
                      ? translateText("Ishdan chiqgan")
                      : "Ishdan chiqgan"
                  }}
                </th>
                <th class="p-3 text-center font-semibold">
                  {{ dat === "datakril" ? translateText("Ishchi kuni") : "Ishchi kuni" }}
                </th>
                <th class="p-3 text-center font-semibold">
                  {{ dat === "datakril" ? translateText("Jarimalar") : "Jarimalar" }}
                </th>
              </tr>
            </thead>
            <tbody>
              <tr
                v-for="history in filteredWorkDays(item.workDays)"
                :key="history.id"
                class="text-center"
              >
                <td colspan="3" class="pt-2">
                  <div
                    class="flex justify-between rounded-md py-2 dark:bg-gradient-to-r from-[#2a3655] to-[#3d4e81] border border-black/50 hover:shadow-blue-500/5 hover:border-white/10 duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-2xl bg-white items-center"
                  >
                    <div class="w-full grid grid-cols-6 gap-2 items-center">
                      ...
                      <div class="text-center text-[16px]">
                        {{ formatNumberWithDots(history.lateDeduction) }}
                        {{ dat === "datakril" ? translateText("so'm") : "so'm" }}
                      </div>
                    </div>
                  </div>
              ...
              <tr v-if="!filteredWorkDays(item.workDays).length">
                <td colspan="5" class="text-center py-4">
                  {{
                    dat === "datakril"
                      ? translateText("Ma'lumot topilmadi")
                      : "Ma'lumot topilmadi"
                  }}
                </td>
              </tr>
          ...
          <div class="flex justify-between items-center mt-4 gap-4">
            <div>
              {{ dat === "datakril" ? translateText("Jami") : "Jami" }}:
              {{ filteredWorkDays(item.workDays).length }}
            </div>
            <button
              @click="downloadExcel"
              class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700"
            >
              {{
                dat === "datakril"
                  ? translateText("Excel qilib yuklab olish")
      ...
      <div
        v-if="salary"
        @click="salary = false"
        class="bg-black inset-0 flex z-40 fixed bg-opacity-70 justify-center items-center"
      >
        <div
          v-if="salary"
          @click.stop
          class="p-6 max-w-md mx-auto bg-white dark:bg-gray-700 rounded-xl w-[400px] shadow-md space-y-4"
        >
          <p class="dark:text-white text-black text-[20px]">
            <strong class="dark:text-white text-black text-[20px]">{{
          ...
          <p class="dark:text-white text-black text-[20px]">
            <strong class="dark:text-white text-black text-[20px]">{{
              dat === "datakril" ? translateText("Jami soat:") : "Jami soat:"
            }}</strong>
            {{ salary.totalHours }}
          </p>
          <p class="dark:text-white text-black text-[20px]">
            <strong class="dark:text-white text-black text-[20px]">{{
              dat === "datakril" ? translateText("Jami maosh:") : "Jami maosh:"
            }}</strong>
            {{ salary.totalSalary }}
          </p>
          <p class="dark:text-white text-black text-[20px]">
          ...
          <p class="dark:text-white text-black text-[20px]">
            <strong class="dark:text-white text-black text-[20px]">{{
              dat === "datakril" ? translateText("Yakuniy maosh:") : "Yakuniy maosh:"
            }}</strong>
            {{ salary.finalSalary }}
          </p>
          <button
            class="w-full py-2 dark:bg-gray-900 bg-gray-300 text-[20px] text-gray-800 dark:text-gray-200 rounded hover:bg-gray-400"
            @click="salary = false"
          >
        ...
        >
          ...
          <div class="space-y-2">
            <label class="block text-md font-medium text-gray-600 dark:text-gray-300">{{
              dat === "datakril" ? translateText("Summa") : "Summa"
            }}</label>
            <input
            ...
            <input
              ...
              class="w-full border border-gray-300 text-black rounded text-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
              :placeholder="
                dat === 'datakril' ? translateText('Sabab kiriting') : 'Sabab kiriting'
              "
            />
          </div>
          <div class="flex justify-end space-x-2">
            <button
              class="px-4 py-2 bg-gray-300 text-xl text-gray-800 rounded hover:bg-gray-400"
              @click="modals = false"
            >
              {{ dat === "datakril" ? translateText("Bekor qilish") : "Bekor qilish" }}
            </button>
            <button
              class="px-4 py-2 bg-blue-600 text-xl text-white rounded hover:bg-blue-700"
              @click="addBonus"
            >
  ...
  <div
    v-if="reject"
    @click="reject = false"
    class="fixed inset-0 bg-black flex justify-center z-40 bg-opacity-70"
  >
    <div
      @click.stop
    ...
    >
      <h1 class="text-2xl border-b-2 p-4 border-gray-500 font-semibold">
        {{
          dat === "datakril" ? translateText("Bonuslar ro'yxati") : "Bonuslar ro'yxati"
        }}
      </h1>
      <button
        @click="reject = false"
      ...
      <div class="p-4 space-y-3">
        <div
          v-for="(item, index) in users[0].Bonus"
          :key="item.id"
        ...
        >
          ...
          <p class="dark:text-green-400 text-green-700 font-semibold text-xl">
            {{ item.amount }} {{ dat === "datakril" ? translateText("so'm") : "so'm" }}
          </p>
        </div>
      </div>
    </div>
  ...
  <div v-if="isModalOpen" class="fixed z-40 inset-0" @click="isModalOpen = false">
    <div
      ...
      :style="{
        top: `${modalPosition.top - 190}px`,
        left: `${modalPosition.left - 700}px`,
      }"
    >
      ...
      <div class="flex gap-2">
        <div class="space-y-4">
          ...
          <div
            class="flex items-center p-3 bg-gray-50 dark:bg-slate-700 rounded-lg dark:border-none border border-gray-800"
          >
            <span class="text-gray-600 dark:text-gray-300 font-medium w-[200px]">{{
              dat === "datakril" ? translateText("Umumiy foyda:") : "Umumiy foyda:"
            }}</span>
            <span class="ml-auto font-semibold text-gray-800 dark:text-white">
              {{ formatNumberWithDots(cards[1].value) }}
              {{ dat === "datakril" ? translateText("so'm") : "so'm" }}
            </span>
          </div>
          <div
          ...
          >
            <span class="text-gray-600 dark:text-gray-300 font-medium w-[200px]">{{
              dat === "datakril"
                ? translateText("Jismoniy shaxs daromad solig'i (JShDS) – 12%")
                : "Jismoniy shaxs daromad solig'i (JShDS) – 12%"
            }}</span>
            <span class="ml-auto font-semibold text-gray-800 dark:text-white">
              {{ formatNumberWithDots(hisoblaFoiz(cards[1].value, 12)) }}
              {{ dat === "datakril" ? translateText("so'm") : "so'm" }}
            </span>
          </div>
          <div
          ...
          >
            <span class="text-gray-600 dark:text-gray-300 font-medium w-[200px]">{{
              dat === "datakril"
                ? translateText("Majburiy pensiya badali (shaxsiy jamg'arma) – 1%")
                : "Majburiy pensiya badali (shaxsiy jamg'arma) – 1%"
            }}</span>
            <span class="ml-auto font-semibold text-gray-800 dark:text-white">
              {{ formatNumberWithDots(hisoblaFoiz(cards[1].value, 1)) }}
              {{ dat === "datakril" ? translateText("so'm") : "so'm" }}
            </span>
          </div>
          <div
            class="flex items-center p-3 bg-green-50 dark:bg-green-900/50 rounded-lg dark:border-none border border-gray-800"
          >
            <span class="text-gray-600 dark:text-gray-300 font-medium w-[200px]">{{
              dat === "datakril" ? translateText("Sof foyda:") : "Sof foyda:"
            }}</span>
            <span class="ml-auto font-bold text-yellow-600 dark:text-yellow-400">
              {{ formatNumberWithDots(cards[1].value - hisoblaFoiz(cards[1].value, 13)) }}
              {{ dat === "datakril" ? translateText("so'm") : "so'm" }}
            </span>
          </div>
        </div>
        <div class="space-y-4">
          ...
          >
            ...
            <span class="ml-auto font-semibold text-gray-800 dark:text-white">
              {{ formatNumberWithDots(cards[2].value) }}
              {{ dat === "datakril" ? translateText("so'm") : "so'm" }}
            </span>
          </div>
          <div
          ...
          >
            ...
            <span class="ml-auto font-semibold text-gray-800 dark:text-white">
              {{ formatNumberWithDots(hisoblaFoiz(cards[2].value, 12)) }}
              {{ dat === "datakril" ? translateText("so'm") : "so'm" }}
            </span>
          </div>
          <div
          ...
          >
            ...
            <span class="ml-auto font-semibold text-gray-800 dark:text-white">
              {{ formatNumberWithDots(hisoblaFoiz(cards[2].value, 1)) }}
              {{ dat === "datakril" ? translateText("so'm") : "so'm" }}
            </span>
          </div>
          <div
          ...
          >
            ...
            <span class="ml-auto font-bold text-yellow-600 dark:text-yellow-400">
              {{ formatNumberWithDots(cards[2].value - hisoblaFoiz(cards[2].value, 13)) }}
              {{ dat === "datakril" ? translateText("so'm") : "so'm" }}
            </span>
          </div>
        </div>
      ...
      <div>
        <div
          class="flex items-center mt-4 p-3 bg-red-50 dark:bg-green-900/50 rounded-lg dark:border-none border border-gray-800"
        >
          <span class="text-gray-600 dark:text-gray-300 font-medium w-[200px]">{{
            dat === "datakril" ? translateText("Umumiy oylik:") : "Umumiy oylik:"
          }}</span>
          <span class="ml-auto font-semibold text-green-600 dark:text-green-400">
            {{
              formatNumberWithDots(
                cards[2].value -
                  hisoblaFoiz(cards[2].value, 13) +
                  cards[1].value -
                  hisoblaFoiz(cards[2].value, 13)
              )
            }}
...
const cards = ref([
  {
    ...
    value: "",
    increase:
      "Belgilangan vaqtdan kechga qolganligi uchun oylikidan kesilgan jarima summasi",
  ...
  },
  {
    ...
    value: "",
    increase:
      "Faollik ko'rsatganligi uchun shu kungacha yigilgan rag'batlantirish summasi",
...
const getSalery = async () => {
  ...
  try {
    ...
    const res = await axios.get(URL + "/salary/preview", {
      params: {
        userId: parseInt(id),
        month: `${year}-${month}`,
      },
...
const downloadExcel = () => {
  ...
  users.value.forEach((item) => {
    ...
    excelRows.push([
      {
        v: `${item.name} ${item.surname} ${item.dadname}`,
    ...
    if (Array.isArray(item.workDays)) {
      item.workDays.forEach((history) => {
        excelRows.push([
          ...
          isoToUzbekistanTime(history.createdAt),
          formatNumberWithDots(history.lateDeduction),
          formatNumberWithDots(history.dailyEarnings),
        ]);
      });
    }
    ...
    // BONUSLAR
    if (Array.isArray(item.Bonus) && item.Bonus.length > 0) {
      excelRows.push([""]);

      // Bonus sarlavhasi
      excelRows.push(["Bonus summasi", "Sanasi"]);

      item.Bonus.forEach((bonus) => {
        excelRows.push([
          formatNumberWithDots(bonus.amount),
          isoToUzbekistanTime(bonus.createdAt),
        ]);
      });
    }

    // Yana bo'sh qator (keyingi user uchun)
    excelRows.push([]);
  });
